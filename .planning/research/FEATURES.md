...
| **Commit → change drill-down** | Users expect to click a claim and see the proof — standard in GitLens Inspect, Repowise evidence drawer, code graph node detail | LOW | Era/summary → commit → file → hunk. Without this, tours are slideshows, not archaeology |
...
| **Incremental re-index** | Stale context is worse than none (Meta/Riftmap research); Repowise syncs per commit, comparable auto-update tooling | MEDIUM–HIGH | Re-process only new commits; invalidate affected eras/decisions. Required for "living" onboarding artifact |
...
| **IDE / agent integration surface** | Target users live in Cursor/Claude Code; Repowise MCP, IDE slash commands, cADR agent skill set expectation | MEDIUM | Slash commands + queryable index artifacts. Host AI is the LLM — tool exposes structured context, not a chat UI |
...
| **Exportable / persistent artifacts** | Onboarding briefs must survive the session — Repowise `.repowise/`, comparable knowledge-graph artifacts, git-story HTML/JSON | LOW | Generated index under `.gitchange/`; optional commit to repo. Users expect to share a link or file, not re-run analysis every time |
...
| **Topic-thread tours** | Cross-cutting concerns (auth, payments, migration X) span eras — no competitor does this cleanly; structural graph tours, not temporal threads | HIGH | Thread = ordered stops across eras/commits/files for one concern. High value for "how did auth evolve?" |
...
| **Temporal knowledge graph** | Files ↔ authors ↔ commits ↔ decisions ↔ eras in one navigable graph — Codebase Time Machine has this; Repowise has separate layers; GitChange unifies **time** as first-class dimension | HIGH | D3/similar interactive graph with time slider or era coloring. Structural graphs (StakGraph and similar) are adjacent, not substitutes |
...
| **Tour player in local dashboard** | IDE plugin pattern applied to **temporal** narrative — combined tour + timeline + graph in one UI | MEDIUM–HIGH | `/gitchange-dashboard` local server; era → commit → file drill-down in player. UX differentiation vs CLI-only archaeology tools (git-story, repo-saga) |
...
| **Plugin artifact pattern (`.gitchange/`)** | Team-opt-in committed index, agent-readable — proven by `.gitchange/`-style artifact folders and Repowise `.repowise/` | LOW–MEDIUM | Enables CI refresh, PR review of lore, agent queries without re-index. Distribution pattern, not user-facing feature — but enables team workflows competitors scatter |
...
| **Full static dependency / call graph** | "Show me everything" | Repowise, Repowise, StakGraph own this space; massive scope creep for temporal product | Temporal edges (co-change, co-authorship, decision governance) — not AST call graphs in v1 |
...
| **Forking external plugin codebases** | Faster start | Wrong abstraction (structural vs temporal); merge burden; PROJECT.md rejected | Inspired-by plugin packaging only; separate codebase |
...
| Feature | Repowise | Vestige | Codebase Time Machine | Historex / git-story | Structural graph tools | GitChange Approach |
|---------|----------|---------|----------------------|----------------------|---------------------|-------------------|
