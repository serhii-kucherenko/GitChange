...
**Rationale:** Mirrors standard proven plugin monorepo without forking it. `core` exposes browser-safe subpath exports (`./types`, `./schema`, `./search`) so the dashboard never imports Node-only modules (es-git, better-sqlite3).
